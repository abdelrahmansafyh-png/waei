  ...
  if (loading) {
    return (
      <ChildLayout profile={profile} activeHref="/dashboard/certificates">
        <div className="flex min-h-screen flex-1 items-center justify-center text-2xl font-black text-[#7050E8]">
          جاري تحميل الشهادات...
        </div>
      </ChildLayout>
    );
      ...
      <main className="min-h-screen flex-1 bg-cover bg-fixed bg-center px-4 py-6 md:px-8">
        <div className="mx-auto max-w-6xl">
          <header className="mb-7 overflow-hidden rounded-[2.8rem] border border-white/40 bg-white/92 p-7 shadow-[0_22px_70px_rgba(62,87,120,.18)] backdrop-blur-xl">
            <div className="flex flex-wrap items-center justify-between gap-5">
              <div>
                ...
                <h1 className="text-4xl font-black leading-[1.2] text-[#20294f] md:text-5xl">
              ...
              <div className="flex items-center gap-4 rounded-[2rem] bg-[#F9FFFD] p-4 shadow-sm">
                <div className="flex h-16 w-16 items-center justify-center rounded-[1.4rem] bg-white text-4xl shadow-sm">
                  {childAvatar}
                </div>
                <div>
                  <div className="text-xl font-black text-[#0B4D6B]">{childName}</div>
          ...
          {certificates.length ? (
            <div className="grid gap-5 lg:grid-cols-2">
              {certificates.map((certificate) => (
                <section
                  ...
                  className="relative overflow-hidden rounded-[2.4rem] border border-[#DDEDEA] bg-white p-7 shadow-[0_18px_45px_rgba(62,87,120,.10)]"
                >
                  <div className="absolute -left-10 -top-10 h-32 w-32 rounded-full bg-[#E8F8F3]" />
                  <div className="absolute -bottom-12 -right-12 h-36 w-36 rounded-full bg-[#F3EFFF]" />

                  <div className="relative">
                    <div className="mb-5 flex items-center justify-between gap-4">
                      <div className="rounded-full bg-[#E8F8F3] px-5 py-2 text-sm font-black text-[#0E9FAA]">
                        شهادة إتمام
                      </div>
                      <div className="text-4xl">🏆</div>
                    </div>

                    <div className="rounded-[2rem] border-2 border-dashed border-[#DDEDEA] bg-[#FAFFFD] p-6 text-center">
                      <div className="text-sm font-black text-[#667085]">تشهد منصة راشد أن</div>
                      <div className="mt-3 text-3xl font-black text-[#0B4D6B]">{childName}</div>
                      <div className="mt-4 text-sm font-black text-[#667085]">أكمل برنامج</div>
                      <div className="mt-2 text-2xl font-black text-[#7050E8]">
                        {certificate.program_title}
                      </div>

                      <div className="mt-6 grid gap-3 sm:grid-cols-2">
                        <div className="rounded-2xl bg-white p-4">
                          <div className="text-sm font-black text-[#667085]">وقت التعلم</div>
                          <div className="mt-1 font-black text-[#0B4D6B]">
                            {formatDuration(certificate.elapsed_seconds)}
                          </div>
                        </div>
                        <div className="rounded-2xl bg-white p-4">
                          <div className="text-sm font-black text-[#667085]">تاريخ الإكمال</div>
                          <div className="mt-1 font-black text-[#0B4D6B]">
                    ...
                    <div className="mt-5 flex flex-wrap gap-3">
                      ...
                      <button
                        ...
                        className="rounded-full bg-[#E8F8F3] px-6 py-3 font-black text-[#0B4D6B]"
                      >
                        طباعة الشهادة
                      </button>
                    </div>
            ...
            <div className="rounded-[2.5rem] border-2 border-dashed border-[#DDEDEA] bg-white/92 p-12 text-center shadow-[0_18px_45px_rgba(62,87,120,.08)]">
              <div className="text-6xl">🎓</div>
              <h2 className="mt-5 text-3xl font-black text-[#0B4D6B]">لا توجد شهادات بعد</h2>
              <p className="mx-auto mt-3 max-w-xl font-bold leading-8 text-[#667085]">
                عند إنهاء برنامج كامل ستظهر شهادته هنا مع اسم البرنامج، وقت التعلم، وتاريخ الإكمال.
              </p>
              <Link
                href="/child/programs"
                className="mt-6 inline-flex rounded-full bg-[#7050E8] px-8 py-4 font-black text-white"
              >
                الذهاب إلى البرامج
              </Link>
            </div>
