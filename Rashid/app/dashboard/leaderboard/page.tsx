    ...
    <ParentLayout>
      <div className="mx-auto max-w-6xl space-y-7">
          ...
          {[
            ...
            <button
              ...
              className={`rounded-full px-5 py-3 font-black shadow-sm transition ${
                sortBy === key
                  ? "bg-[var(--rashid-color-42bfa8)] text-white"
                  : "bg-white text-[var(--rashid-color-0b4d6b)] hover:bg-[var(--rashid-color-ecfbf7)]"
              }`}
            >
              {label}
            </button>
        ...
        <section className="rounded-[2.5rem] bg-white p-6 shadow-[0_18px_45px_rgba(18,34,74,.08)]">
          {loading ? (
            <div className="p-10 text-center text-2xl font-black text-[var(--rashid-color-7048e8)]">جاري تحميل الترتيب...</div>
          ) : leaderboard.length ? (
            <div className="space-y-4">
              {leaderboard.map((child, index) => (
                <Link
                  ...
                  className="flex flex-wrap items-center justify-between gap-4 rounded-[1.8rem] bg-[var(--rashid-color-f9fffd)] p-5 transition hover:-translate-y-1 hover:bg-[var(--rashid-color-f1fffb)]"
                >
                  <div className="flex items-center gap-4">
                    <div className={`grid h-14 w-14 place-items-center rounded-2xl text-2xl font-black ${
                      index === 0 ? "bg-[var(--rashid-color-ffd54a)] text-[var(--rashid-color-5a3a00)]" : index === 1 ? "bg-[var(--rashid-color-e6eef3)] text-[var(--rashid-color-0b4d6b)]" : index === 2 ? "bg-[var(--rashid-color-ffe4c2)] text-[var(--rashid-color-b75a00)]" : "bg-white text-[var(--rashid-color-0b4d6b)]"
                    }`}>
                      {index + 1}
                    </div>
                    <div className="text-4xl">{child.gender === "female" ? "👧" : "👦"}</div>
                    <div>
                      <div className="text-xl font-black text-[var(--rashid-color-0b4d6b)]">{child.full_name || "طفل"}</div>
                      <div className="mt-1 text-sm font-bold text-[var(--rashid-color-667085)]">العمر: {child.age || "-"} سنوات</div>
                    </div>
                  </div>
                  ...
                  <div className="grid grid-cols-3 gap-3 text-center">
                    <div className="rounded-2xl bg-white px-4 py-3">
                      ...
                      <div className="text-xs font-bold text-[var(--rashid-color-667085)]">XP</div>
                    </div>
                    <div className="rounded-2xl bg-white px-4 py-3">
                      <div className="font-black text-[var(--rashid-color-0b4d6b)]">✅ {child.completed_programs}</div>
                      <div className="text-xs font-bold text-[var(--rashid-color-667085)]">برامج</div>
                    </div>
                    <div className="rounded-2xl bg-white px-4 py-3">
                      <div className="font-black text-[var(--rashid-color-0b4d6b)]">⏱️ {formatDuration(child.total_time_seconds)}</div>
                      <div className="text-xs font-bold text-[var(--rashid-color-667085)]">وقت</div>
                    </div>
                  </div>
                </Link>
              ))}
            ...
            <div className="rounded-[2rem] border-2 border-dashed border-[var(--rashid-color-ddedea)] p-10 text-center font-black text-[var(--rashid-color-667085)]">
