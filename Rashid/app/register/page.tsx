  ...
  return (
    <main dir="rtl" className="relative min-h-screen overflow-hidden bg-[var(--rashid-color-f5fbff)] text-[var(--rashid-color-14224a)]">
      <style>{`
        ...
        .auth-input{height:58px;width:100%;border-radius:22px;border:1px solid rgba(14,159,170,.18);background:rgba(255,255,255,.92);padding:0 18px;font-weight:800;outline:none;color:var(--rashid-color-14224a);box-shadow:0 10px 28px rgba(20,34,74,.06);transition:.2s ease}
      ...
      <div className="relative z-10 mx-auto grid min-h-screen max-w-7xl items-center gap-8 px-5 py-8 lg:grid-cols-[.88fr_1fr]">
        <section className="hidden lg:block auth-fade-up">
          <div className="relative overflow-hidden rounded-[46px] auth-glass p-10">
            ...
            <div className="mt-10 inline-flex rounded-full bg-white/80 px-5 py-2 text-sm font-black text-[var(--rashid-color-0e9faa)] shadow-lg">حساب ولي الأمر أولًا 🌱</div>
            <h1 className="mt-7 text-6xl font-black leading-[1.2] tracking-[-.03em] text-[var(--rashid-color-101b3d)]">
              أنشئ حسابك
              <br />
              وأضف أبناءك من
              <span className="text-[var(--rashid-color-0e9faa)]"> لوحة التحكم</span>
            </h1>
            ...
            <div className="mt-10 space-y-4">
              {[["👨‍👩‍👧", "حساب ولي أمر واحد لإدارة الأبناء"], ["🔐", "دخول الطفل بكود بسيط من ولي الأمر"], ["🏆", "متابعة XP والبرامج والشهادات"]].map(([icon, text]) => (
                <div key={text} className="flex items-center gap-4 rounded-[26px] bg-white/76 p-5 shadow-lg">
                  <span className="flex h-14 w-14 items-center justify-center rounded-2xl bg-gradient-to-br from-[var(--rashid-color-19c6d4)] to-[var(--rashid-color-0e9faa)] text-3xl shadow-lg">{icon}</span>
                  <span className="text-lg font-black text-[var(--rashid-color-14224a)]">{text}</span>
                </div>
              ))}
            </div>
          </div>
        ...
        <form onSubmit={handleRegister} className="relative mx-auto w-full max-w-[620px] overflow-hidden rounded-[42px] auth-glass p-6 md:p-9 auth-fade-up">
          ...
          <div className="relative z-10">
            <div className="mb-8 flex items-center justify-between gap-4">
              ...
              <button type="button" onClick={() => router.push("/login")} className="rounded-full bg-white/80 px-5 py-3 text-sm font-black text-[var(--rashid-color-0e9faa)] shadow-lg transition hover:-translate-y-1">لدي حساب</button>
            </div>

            <div className="mb-7">
              <p className="text-sm font-black text-[var(--rashid-color-0e9faa)]">تسجيل ولي أمر</p>
              <h2 className="mt-2 text-3xl font-black text-[var(--rashid-color-101b3d)] md:text-4xl">بيانات ولي الأمر</h2>
              <p className="mt-3 font-bold leading-8 text-[var(--rashid-color-566681)]">بعد إنشاء الحساب ستتمكن من إضافة الأطفال وإعطائهم اسم مستخدم وكود دخول.</p>
            </div>
            ...
            <div className="space-y-4">
              ...
              <div className="grid grid-cols-3 gap-3">
                {[["father", "أب", "👨"], ["mother", "أم", "👩"], ["guardian", "وصي", "🧑"]].map(([value, label, icon]) => (
                  <button key={value} type="button" onClick={() => setGuardianType(value as GuardianType)} className={`rounded-[24px] border-2 p-4 font-black transition ${guardianType === value ? "border-[var(--rashid-color-19c6d4)] bg-[var(--rashid-color-e9fbfc)] text-[var(--rashid-color-14224a)] shadow-lg" : "border-white/70 bg-white/75 text-[var(--rashid-color-566681)]"}`}>
              ...
              <button disabled={loading} className="mt-3 w-full rounded-full bg-gradient-to-r from-[var(--rashid-color-19c6d4)] to-[var(--rashid-color-0e9faa)] py-4 font-black text-white shadow-xl shadow-[var(--rashid-color-19c6d4)]/25 transition hover:-translate-y-1 disabled:opacity-50">
                {loading ? "جاري الإنشاء..." : "إنشاء حساب ولي الأمر"}
              </button>

              <Link href="/child-login" className="block text-center font-black text-[var(--rashid-color-8b5cf6)] transition hover:text-[var(--rashid-color-6847f5)]">
