...
export default function ParentDashboard({ profile }: { profile: any }) {
  ...
  const totalMinutes = useMemo(
    () =>
      Math.round(
        attempts.reduce((sum, attempt) => sum + (attempt.duration_seconds || 0), 0) / 60
      ),
    [attempts]
  );
        ...
        <section className="mt-10">
          <div className="mb-6 flex flex-wrap items-end justify-between gap-4">
            <div>
              ...
              <h2 className="text-4xl font-black text-[#0B4D6B]">أبنائي</h2>
            </div>

            <Link
              href="/dashboard/children/new"
              className="rounded-full bg-[#42BFA8] px-6 py-3 font-black text-white shadow-lg transition hover:-translate-y-1"
            >
              + إضافة طفل
            </Link>
          </div>
            ...
            <div className="grid gap-6 md:grid-cols-2 xl:grid-cols-3">
              {children.slice(0, 6).map((child) => {
                const childAttempts = attempts.filter(
                  (attempt) => attempt.child_profile_id === child.id
                );
